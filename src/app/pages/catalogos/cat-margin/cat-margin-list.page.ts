...
import { ICatExperiences, CatExperiences } from 'app/models/catalogos/cat-experiences.model';
import { CatMargin } from 'app/models/catalogos/cat-margin.model';
...
import { CatExperiencesDialogPage } from '../cat-experiences/cat-experiences-dialog.page';
import { CatMarginService } from '../../../services/catalogos/cat-margin.service';
import { CatMarginDialogPage } from './cat-margin-dialog.page';

@Component({
  selector: 'app-cat-margin-list',
  templateUrl: './cat-margin-list.page.html'
})
export class CatMarginListPage implements OnInit {
  totalItems = 0;
  ...
  constructor(
      ...
      private valueService: CatMarginService
  ) {
  ...
  newItem(): void {
      ...
      const value: ICatExperiences = new CatExperiences();
      const params = {
        value,
        title: this.translate.instant('CATALOGS.DIALOG.TITLE_CREATE'),
      };
      const dialogRef = this.dialog.open(CatMarginDialogPage, { data: params });
  ...
  view(value: CatMargin): void {
      let data = {
        title: this.translate.instant('CATALOGS.DIALOG.TITLE_UPDATE') ,
        catExperiences: value,
      };
      this.dialogConf.data = data;
  
      const dialogRef = this.dialog.open(CatMarginDialogPage,  this.dialogConf );
      dialogRef.updateSize('50%');
      dialogRef.afterClosed().subscribe((src: any) => {
