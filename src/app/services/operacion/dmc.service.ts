...
export class DmcService {
  public resourceUrl = environment.apiUrl + '/api/dmc/collections';
  ...
  queryByCollections(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
  ...
  protected convertDateFromClient(scr: ICollections): ICollections {
    const copy: ICollections = Object.assign({}, scr, {
      ...
      ending_date: moment(scr.starting_date).isValid() ?
      moment(scr.starting_date).format(DATE_FORMAT) : undefined,
