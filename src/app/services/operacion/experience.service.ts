...
import { IExperience } from 'app/models/core/experience.model';
...
export class ExperienceService {
  public resourceUrl = environment.apiUrl + '/api/experience';

  constructor(protected http: HttpClient) { }

  create(hotel: IExperience): Observable<EntityResponseType> {
  ...
  update(hotel: IExperience): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(hotel);
    return this.http
      .put<IExperience>(`${this.resourceUrl}/${copy.id}`, copy, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }
  ...
  queryByflight(id: number, req?: any): Observable<EntityArrayResponseType> {
    ...
    const options = createRequestOption(req);
    return this.http
      .get<IExperience[]>(`${this.resourceUrl}/all/${id}`, { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient(hotel: IExperience): IExperience {
    const copy: IExperience = Object.assign({}, hotel, {
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    ...
    if (res.body && res.body.length > 0) {
      res.body.forEach((hotel: IExperience) => {
